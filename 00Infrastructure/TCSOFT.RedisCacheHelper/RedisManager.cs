...
namespace TCSOFT.RedisCacheHelper
{
    ...
    internal class RedisManager
    {
        ...
        /// </summary>
        internal static string RedisSysCustomKey = string.Empty; //ConfigurationManager.AppSettings["RedisSysCustomKey"];
        /// <summary>
        /// 当前连接的Redis中的DataBase索引，默认0-16，可以在service.conf配置，最高64
        /// </summary>
        internal static int RedisDataBaseIndex = 0; // int.Parse(ConfigurationManager.AppSettings["RedisDataBaseIndex"]);
        /// <summary>
        /// 当前连接的Redis中连接字符串，格式为：127.0.0.1:6379,allowadmin=true,passowrd=pwd
        /// </summary>
        internal static string RedisHostConnection = string.Empty; // ConfigurationManager.AppSettings["RedisHostConnection"];
                ...
                if (_instance == null)
                {
                    lock (Locker)
                    {
                        if (_instance == null || !_instance.IsConnected)
                        {
                            ...
                            RedisSysCustomKey = configuration["RedisInfo:RedisInstanceCustomKey"];
                            RedisDataBaseIndex = int.Parse(configuration["RedisInfo:RedisDataBaseIndex"]);
                            RedisHostConnection = configuration["RedisInfo:RedisHostConnection"];
        ...
        private static ConnectionMultiplexer GetManager(string connectionString = null)
        {
            ...
            using (var discoverCnn = ConnectionMultiplexer.Connect(connectionString))
            {
                connectionString = string.Join(",", discoverCnn.GetEndPoints(false).Select(ep => ep.ToString()));
            }
